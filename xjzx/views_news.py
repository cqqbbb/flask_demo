...
@news_bp.route('/comment/add', methods=['POST'])
def commentadd():
    dict1 = request.form
...
@news_bp.route('/comment/list/<int:news_id>')
def commentlist(news_id):
    comment_list = NewsComment.query.filter_by(news_id=news_id,comment_id=None).order_by(NewsComment.id.desc())
    comment_list2 = []
    if 'user_id' in session:
        user_id = session['user_id']
...
@news_bp.route('/comment/up/<int:comment_id>', methods=['POST'])
...
@news_bp.route('/comment/back', methods=['POST'])
def commentback():
    news_id = request.form.get('news_id')
...
@news_bp.route('/follow',methods=['POST'])
def follow():
